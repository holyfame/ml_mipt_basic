...
def rbf(x_1, x_2, sigma=1.):
    ...
    x_1_size, x_2_size = x_1.shape[0], x_2.shape[0]
    distances = np.zeros((x_1_size, x_2_size))
    for i in range(x_1_size):
      for j in range(x_2_size):
        distances[i][j] = np.exp(-np.linalg.norm(x_1[i] - x_2[j]) ** 2 / (2 * sigma ** 2))
...
def hinge_loss(scores, labels):
    ...
    return torch.mean(torch.max(torch.zeros_like(scores), 1 - scores * labels)) ### YOUR CODE HERE
